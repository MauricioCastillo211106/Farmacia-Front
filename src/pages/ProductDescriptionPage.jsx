...
const ProductDescriptionPage = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      clienteId = decodedToken.employee_id;
    ...
    .then(data => {
      ...
      const existingCart = data.find(cart => cart.cliente_id === clienteId);
      if (existingCart) {
  ...
  const handleAddToCart = () => {
    ...
    if (!cartId) {
      ...
      .then(newCart => {
        ...
        if (!newCart.id) {
          console.error("ID del carrito no encontrado en la respuesta:", newCart);
  ...
  const verifyCartAssociation = (clienteId, createdCartId) => {
    ...
    .then(data => {
      ...
      const verifiedCart = data.find(cart => cart.cliente_id === clienteId && cart.id === createdCartId);
      if (verifiedCart) {
        ...
        <>
          <div className="product-detail">
            ...
            <div className="product-details-content">
              ...
              <Text><strong>Fórmula:</strong> {product.formula}</Text>
              <Text><strong>Efectos Secundarios:</strong> {product.secundary_effects}</Text>
              <Text><strong>Caducidad:</strong> {new Date(product.caducity).toLocaleDateString()}</Text>
              <Text><strong>Dosis:</strong> {product.dose}</Text>
              <Text><strong>Clasificación:</strong> {product.type}</Text>
              <Text><strong>Uso:</strong> {product.uso}</Text>
